...
def read_glove(file_name, word2idx):
    word_emb = []
    f = open(file_name, 'r+', encoding="utf-8")
    for line in f:
        wtf = line.strip().split(' ')
        word = wtf[0]
        emb = [float(r) for r in wtf[1:]]
        if word == 'unk':
            word_emb = [emb] + word_emb
        if word in word2idx:
            word_emb.append(emb)
    return word_emb

def add_start_end(data, word2idx, pos2idx, label2idx):
    tmp = []
    for doc in data:
        tmp_sent, tmp_poss, tmp_labels = [], [], []
        for sent in doc[1][0]:
            tmp_sent.append([word2idx['<start>']] + sent + [word2idx['<end>']])
        for poss in doc[1][1]:
            tmp_poss.append([pos2idx['<start>']] + poss + [pos2idx['<end>']])
        for labels in doc[1][2]:
            tmp_labels.append([label2idx['<start>']] + labels + [label2idx['<end>']])
        tmp.append([doc[0], [tmp_sent, tmp_poss, tmp_labels]])
    return tmp
...
def generate_corpus(lines, feature_map, label_map, if_shrink_feature=False, thresholds=1):
    ...
    for line in lines:
        if not (line.isspace() or (len(line) > 10 and line[0:10] == '-DOCSTART-')):
            ...
            tmp_ll.append(line[-1].upper())
            if line[-1].upper() not in label_map:
                label_map[line[-1].upper()] = len(label_map)
        elif len(tmp_fl) > 0:
...
def load_embedding_wlm(emb_file, delimiter, feature_map, full_feature_set, caseless, unk, emb_len, shrink_to_train=False, shrink_to_corpus=False):
    ...
    #ensure <unk> is 0
    word_dict = {v:(k+1) for (k,v) in enumerate(sorted(feature_set - set(['<unk>'])))}
    word_dict['<unk>'] = 0
...
def construct_bucket_mean_vb_wc(word_features, input_label, corpus_missing_tagspace, label_dict, char_dict, word_dict, caseless, corpus_mask_value=0.0):
    """
    ...
    return construct_bucket_vb_wc(word_features, forw_features, fea_len, labels, thresholds, word_dict['<eof>'], char_dict['\n'], label_dict['<pad>'], len(label_dict), corpus_missing_tagspace, corpus_mask_value)

def construct_bucket_vb_wc(word_features, forw_features, fea_len, input_labels, thresholds, pad_word_feature, pad_char_feature, pad_label, label_size, corpus_missing_tagspace, corpus_mask_value=0.0):
    """
    ...
    # calc padding
    for i, (f_f, f_l, w_f, i_l) in enumerate(zip(forw_features, fea_len, word_features, input_labels)):
        cur_len = len(f_l)
        idx = 0
        cur_len_1 = cur_len + 1
        while thresholds[idx] < cur_len_1:
        ...
        corpus_mask = torch.FloatTensor(label_size, label_size).fill_(1.0)
        if corpus_missing_tagspace and len(corpus_missing_tagspace) > i and corpus_missing_tagspace[i]:
            corpus_mask[corpus_missing_tagspace[i]] = corpus_mask_value
            corpus_mask[:,corpus_missing_tagspace[i]] = corpus_mask_value
        buckets[idx][8].append(corpus_mask)
        
        # reorder
        buckets[idx][9].append(i)
    ...
    bucket_dataset = [CRFDataset_WC(torch.LongTensor(bucket[0]), torch.LongTensor(bucket[1]),
                                   ...
                                   torch.stack(bucket[8]), bucket[9]) for bucket in buckets]
...
def iobes2spans(sequence, strict_iob2=False):
    """
    convert to iobes to span
    """
    iobtype = 2 if strict_iob2 else 1
